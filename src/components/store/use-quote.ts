 ...
 */
export function useQuote(lines: CartLine[], opts: { region?: string | null; code?: string | null; email?: string | null } = {}) {
  ...
  const key = JSON.stringify([lines, opts.region ?? null, opts.code ?? null, opts.email ?? null]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      startTransition(async () => {
        const result = await getQuote({ lines, region: opts.region, code: opts.code, email: opts.email });
